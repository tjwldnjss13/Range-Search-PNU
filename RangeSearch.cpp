...
std::vector<Node*> find_nodes_in_range_x(RedBlackTreePnt range_tree_x, std::pair<int, int> range_x, std::pair<int, int> range_y) {
	...
	while (t_l != NULL) {
		if (t_l->pnt[0].x < range_x.first)		
			...
			for (int n_i = 0; n_i < nodes_subtree.size(); n_i++) {
	...
	while (t_r != NULL) {
		if (t_r->pnt[0].x > range_x.first)		t_r = t_r->left;
		else {
			nodes_subtree.clear();
			nodes.push_back(t_r);
			nodes_subtree.clear();
...
std::vector<Node*> find_nodes_in_range_y(RedBlackTreePnt range_tree_y, std::pair<int, int> range_y) {
	...
	if (cur != NULL)		return nodes;

	Node *splitv = NULL;

	while (cur != NULL) {
		if (cur->pnt[0].x < range_y.first)			cur = cur->right;
		else if (cur->pnt[0].y > range_y.second)	cur = cur->left;
		else {
			splitv = cur;
			break;
		}
	...
	while (t_l != NULL) {
		if (t_l->pnt[0].y < range_y.first)		t_l = t_l->right;
		else {
			nodes_subtree.clear();
			nodes.push_back(t_l);
			...
			if (t_l->pnt[0].x == range_y.first)		break;
			else									t_l = t_l->left;
		}
	}

	while (t_r != NULL) {
		if (t_r->pnt[0].x > range_y.first)		t_r = t_r->left;
		else {
			nodes_subtree.clear();
			nodes.push_back(t_r);
			get_nodes_in_subtree(t_r->left, nodes_subtree);
			...
			if (t_r->pnt[0].x == range_y.second)		break;
			else										t_r = t_r->right;
		}
	}
